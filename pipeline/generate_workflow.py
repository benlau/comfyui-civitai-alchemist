...
def _add_markdown_note(workflow: dict, metadata: dict, node_id: str = "100") -> None:
    """
    Add a MarkdownNote node displaying the source image URL.

    Args:
        workflow: Workflow dict to mutate
        metadata: Image metadata with image_url or image_id
        node_id: Node ID to use (default "100" to avoid collision)
    """
    image_id = metadata.get("image_id", "")

    proj_url = "https://github.com/ThePhilosopherStone/comfyui-civitai-alchemist"

    text = textwrap.dedent(f"""\
        This workflow is generated by [comfyui-civitai-alchemist]({proj_url}) based
        on the [Reference Image](https://civitai.com/images/{image_id}).""")
...
def _build_txt2img_workflow(metadata: dict, resources: dict,
    ...
    _add_markdown_note(workflow, metadata)
